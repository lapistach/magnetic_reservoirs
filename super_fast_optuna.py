...
def main():
    ...
    file_path = r"Y:\SOT_lab\People\Dashiell\fast_codes_folder\while_gone_unsure" # you must create the folders before writing the path
    study_name = 'mnist_try_test' # choose a study name
    ...
    X_train_before, X_test_before, y_train, y_test, _, _ = ds.mnist_model(samples_number, test_ratio, crop=False)
    X_train_copy = X_train_before.copy()
    X_test_copy = X_test_before.copy()

    ######## Add features, if no features to be added, set n_features=1 ########
    n_features = 2 # also good for output layer.
    X_train, _ = hf.add_features(X_train_before, n_features)
    X_test, _ = hf.add_features(X_test_before, n_features)
    ...
    def objective(trial):
        ...
        B_x = trial.suggest_float("B_x", -3.0, -2.0, step=0.1) 
        forward_ratio = trial.suggest_float("forward_ratio", 0.65, 1.0, step=0.05) 
        ...
        ######## Run the super fast trial ########
        X_total = np.concatenate((X_train, X_test))
        psw_total = super_fast(t_quantized, V_max, forward_ratio, X_total, n_features)
